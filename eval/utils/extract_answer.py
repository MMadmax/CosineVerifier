import math
import re
from utils.math_dapo import normalize_final_answer
...
def extract_answer(solution_str: str):
    ...
    # answer_pattern1 = r"(?i)Answer\s*:\s*([^\n]+)"
    # answer_pattern2 = r"(?i)(?:\*\*)?Final Answer(?:\*\*)?\s*:\s*([^\n]+)"
    # match1 = re.findall(answer_pattern1, solution_str)
    # match2 = re.findall(answer_pattern2, solution_str)
    # # print(match2)
    # if len(match1) >= 1:
    #     return normalize_final_answer(match1[-1])
    # if len(match2) >= 1:
    #     pattern = r"\*\*(.*?)\*\*"
    #     match3 = re.findall(pattern, match2[-1])
    #     if len(match3) > 0:
    #         return match3[-1]
    #     return match2[-1]
